...
class FanzaScrape(BaseJavScrape):
    ...
    def _scrape_by_num(self, num) -> Optional[MovieInfo]:
        raise RuntimeError("暂未实现")

    def get_urls_of_cid(self, cid: str) -> Tuple[str, str]:
        """搜索cid可能的影片URL"""
        r = self.get(
        ...
        if cid not in parsed_result:
            if len(result) > 0:
                logging.debug(f"Unknown URL in search result: " + ', '.join(result))
            raise RuntimeError("404")
        sorted_result = sort_search_result(parsed_result[cid])
        return sorted_result

    def parse_videoa_page(self, movie: MovieInfo, html):
        """解析AV影片的页面布局"""
        ...
        try:
            ...
            logging.debug('解析视频地址时异常: ' + repr(e))
        ...
        if r0.status_code == 404:
            ...
            for d in urls:
                ...
                if func_name in globals():
                    ...
                    logging.debug(f"不知道怎么解析 fanza {d['type']} 的页面: {d['url']}")
                    continue
                r = self.get(d['url'])
                html = resp2html_wrapper(r)
                try:
                    ...
                    logging.debug(f"Fail to parse {d['url']}", exc_info=True)
                    if d is urls[-1]:
                        logging.warning(f"在fanza查找到的cid={movie.cid}的影片页面均解析失败")
                        raise
        else:
...
def resp2html_wrapper(resp):
    ...
    if 'not available in your region' in html.text_content():
        ...
        raise RuntimeError('FANZA要求当前IP登录账号才可访问，请尝试更换为日本IP')
    return html
